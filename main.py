...
from config import SAMPLE_CSV_PATH, ROUTES  # noqa: E402


def collect_and_store(force_refresh: bool = False, live: bool = False) -> None:
    ...
    if live:
        collect_live_ixigo(force_refresh=force_refresh)
        return
...
def collect_live_ixigo(force_refresh: bool = False, date: str | None = None) -> None:
    """
    ...
    travel_date = date or (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
...
if __name__ == "__main__":
    ...
    collect_and_store(force_refresh=args.refresh, live=args.live)
    run_pipeline()
